...
タグ層が無い索引に対しては **1**。`sagasu tags`(引数なし)がタグの無い索引で
1 を返すのと同じ状況で、browse にとってはファセット木がまったく存在しないケースに
あたる。0 を返すと、スクリプトから叩いたときに「探索するものが無い」が
「空の答えを正常に返した」と区別できなくなる。それ以外は 0
(タグの解析エラー等は従来どおり `error:` を出して 1)。
