...
もうひとつのルールは **prune**: 絶対パス**接頭辞**の下には**降りない**。
walker にディレクトリごと Skip させるので、中身は一度も visit されない。
中身を数えないのも同じ理由 — 降りないから数えられない。この違いが
「遅いだけ」と「危険」を分ける。

Linux の既定は擬似ファイルシステムだけ:

```
/proc  /sys  /dev  /run
```

`/proc` `/sys` を歩くのは無駄なだけではない。後の `sagasu fulltext` は索引に入った
ファイルを**開いて読む**。`/proc/kmsg` の類は読み込みで永久にブロックする —
ハングであって、スローダウンではない。
...
`--exclude-prefix <PATH>` は `--exclude`(ディレクトリ名)の兄弟で、絶対パス。
`--no-default-excludes` で既定のプレフィックスも落ちる(ユーザー追加は残る)。
macOS の既定は `/dev`(devfs)のみ。

比較は文字列ではなく**パス比較**(成分単位)。`/sys` は `/system` に一致しない。
Windows では ASCII 大文字小文字を無視する(`delta::path_under` と同じ判断)。

**渡したプレフィックスは canonicalize してから保存・比較する。** 走査側のパスは
canonicalize 済みのルートから降りてくるので、そのまま文字列比較すると**シンボリックリンク
経由の綴りが黙って何も刈らない**。これは些末な例外ではなく、この仕組みが存在する当の
プラットフォームで日常的に起きる — macOS の `/tmp` は `/private/tmp` に解決され、
Windows のジャンクション(`C:\Users\All Users` → `C:\ProgramData`)も同様。実測では
シンボリックリンクのルートに対して**規則を「有効」と表示したまま配下を索引していた**。
表示されるのも保存されるのも解決後の綴りなので、`pruned paths` 行が実際に効いている
場所を指す。

**解決できないプレフィックスは警告する。** 存在しないパス(まだ作られていない、
マウントされていないボリューム)を指す規則はエラーではないが、**黙って no-op になるのは
規則を「有効」と表示することと矛盾する**ので、クロール前に stderr へ出す。値は
入力どおり保存され、次のクロールでその場所が現れれば効く。

prune も数える: `sagasu index` は**降りなかったディレクトリ 1 つ = 1 件**として
`(excluded prefix)` 行に出す(中身の数ではない):
...
ルールは policy に `prefix=` 行として**明示的に**書き出される(「defaults=1」から
推定しない)。形式は **v2** に上がった。この変更前に作られた index の `v1` policy は
**読めるが、prefix 空で復元され新しい既定は適用されない** — クロールと差分が違う集合を
スキャンする不一致を防ぐため(§1-5 と同じ論理)。逆に v2 を古いバイナリが読むと版で
弾かれ、クエリ側は index-only に degrade して stale notice を出す。
