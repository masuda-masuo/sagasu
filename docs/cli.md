...
| 終了コードで「見つからなかった」と「失敗した」を区別 | **今は踏襲していない**(0/1 の 2 値) | §6 |
...
3. **エラーは JSON にしない。** 失敗時は従来どおり stderr に `error: ...` を出して
   終了コード 1。JSON ストリームは途中で終わる。理由: 失敗はほぼ全部 `anyhow` の
...
### 6-2. 判断: rg の 3 値化は**今回やらない**

rg は `0` = マッチあり、`1` = マッチなし、`2` = エラー。sagasu は 1 に
「エラー」と「使える答えが無い」を同居させている。3 値に揃えるのは筋が良いが、
**今回の PR には入れない**:

- 決まっているのは `--json` と設定統合の 2 点で、終了コードは**9 サブコマンド
  全部の契約を同時に変える**。混ぜると重い判断が埋もれる。
- 「マッチなし」の定義がコマンドごとに違う(`search` の 0 件は「索引が空」と
  「ヒット 0」の 2 つがあり、前者は 1 のままにしたい)。この整理自体が設計判断。

**別 issue に切る。** それまでは上の表が仕様で、消費側は「0 なら成功、
それ以外は stderr を読む」で扱う。
...
| 3 | `find` だけ索引が空でも警告も非 0 終了もしない | **PR2 で直す。** `search` と同じ「メタデータ索引が空」警告を追加(終了コードは §6 に従い変えない) |
